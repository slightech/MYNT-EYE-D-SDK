...
void Location::OnGPSDataCallback(const GPSDataPacket& packet) {
  ...
  gps->latitude_degree =
    static_cast<std::uint64_t>(gps->latitude / 100); // separate latitude
  latitude_cent_tmp = gps->latitude - gps->latitude_degree * 100;
  gps->latitude_cent = static_cast<std::uint64_t>(latitude_cent_tmp);
  latitude_second_tmp = (latitude_cent_tmp - gps->latitude_cent) * 60;
  gps->latitude_second = static_cast<std::uint64_t>(latitude_second_tmp);

  gps->longitude_degree =
    static_cast<std::uint64_t>(gps->longitude / 100); // separate latitude
  longitude_cent_tmp = gps->latitude - gps->longitude_degree * 100;
  gps->longitude_cent = static_cast<std::uint64_t>(longitude_cent_tmp);
