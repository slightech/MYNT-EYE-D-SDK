...
class TempWriter {
 public:
  ...
  explicit TempWriter(std::string filename) : filename_(std::move(filename)) {
    ofstream_.open(filename_, std::ofstream::out);
    ofstream_ << "elapsed_seconds, temperature" << std::endl;
    ofstream_ << FULL_PRECISION;
    time_beg_ = times::now();
  }
  ...
  ~TempWriter() {
    ofstream_.flush();
    ofstream_.close();
    std::cout << "Write temp to file: " << filename_ << std::endl;
  }
...
int main(int argc, char const* argv[]) {
...
#ifdef WRITE_TEMP_TO_FILE
  TempWriter temp_writer("sensor_temp.csv");
#endif
  ...
  for (;;) {
    ...
    float temp = cam.GetSensorTemperature();
    std::cout << temp << std::endl;
#ifdef WRITE_TEMP_TO_FILE
    temp_writer.Write(temp);
#endif
  }
